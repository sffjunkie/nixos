...
rec {
  ...
  netdevice =
    config: host: ifname:
    config.looniversity.network.hosts.${host}.netdevice.${ifname}.device;
  ...
  serviceHandler =
    config: serviceHandlerName: config.looniversity.network.serviceHandlers.${serviceHandlerName};

  # serviceHandlerMainPort :: attrSet -> str -> str -> int
  # Get a named port number for the `serviceHandlerName`
  serviceHandlerMainPort =
    config: serviceHandlerName: config.looniversity.network.serviceHandlers.${serviceHandlerName}.port;

  # serviceHandlerNamedPort :: attrSet -> str -> str -> int
  # Get a named port number for the `serviceHandlerName`
  serviceHandlerNamedPort =
    config: serviceHandlerName: portName:
    config.looniversity.network.serviceHandlers.${serviceHandlerName}.ports.${portName};

  # serviceHandlerHostName :: attrSet -> str -> str
  # Get the host name for the `serviceName`
  serviceHandlerHostName =
    config: serviceHandlerName: (serviceHandler config serviceHandlerName).host;

  # serviceHandlerFQDN :: attrSet -> str -> str
  serviceHandlerFQDN =
    config: serviceHandlerName:
    lib.concatStringsSep "." [
      (serviceHandlerHostName config serviceHandlerName)
  ...
  serviceDomainName =
    config: serviceName: config.looniversity.network.services.${serviceName}.domainName;
  ...
  serviceServiceHandlerName =
    config: serviceName: config.looniversity.network.services.${serviceName}.handler;
